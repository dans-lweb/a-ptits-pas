...
function ale_custom_page_links($custom_query) {
	global $wp_rewrite;
...
function get_breadcrumbs() {
	...
	$show_home_link = 1;
	$show_title     = 1;
	$delimiter      = '<span class="fa fa-angle-right arrow"></span>';
	...
	if (is_home() || is_front_page()) {
		...
		if ($show_on_home == 1) echo '<div class="navi"><a href="' . $home_link . '">' . $text['home'] . '</a></div>';

	}
	else {

		echo '<div class="navi" xmlns:v="http://rdf.data-vocabulary.org/#"><span class="fa fa-home"></span>';
		if ($show_home_link == 1) {
...
function get_shop_breadcrumb(){
	...
	$wrap_before  = '<div class="navi"><span class="fa fa-home"></span>';
	$wrap_after   = '</div>';
...
// If permalinks contain the shop page in the URI prepend the breadcrumb with shop
	...
	if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
		...
		if ( is_category() ) {
			...
			echo $prepend . $before . __( 'Products tagged &ldquo;', 'aletheme' ) . $queried_object->name . '&rdquo;' . $after;
			...
			if ( is_search() ) {
				...
				echo $before . '<a href="' . get_post_type_archive_link( 'product' ) . '">' . $_name . '</a>' . $delimiter . __( 'Search results for &ldquo;', 'aletheme' ) . get_search_query() . '&rdquo;' . $after;
			...
			echo $before . __( 'Error 404', 'aletheme' ) . $after;
			...
			echo $before . __( 'Search results for &ldquo;', 'aletheme' ) . get_search_query() . '&rdquo;' . $after;

		} elseif ( is_tag() ) {

			echo $before . __( 'Posts tagged &ldquo;', 'aletheme' ) . single_tag_title('', false) . '&rdquo;' . $after;

		} elseif ( is_author() ) {

			$userdata = get_userdata($author);
			echo $before . __( 'Author:', 'aletheme' ) . ' ' . $userdata->display_name . $after;

		}

		if ( get_query_var( 'paged' ) )
			echo ' (' . __( 'Page', 'aletheme' ) . ' ' . get_query_var( 'paged' ) . ')';
 ...
 */
function twitt(){
	//Get the last Tweet. Using the Recent Tweets Widget
	if (class_exists('tp_widget_recent_tweets')) {
		$recent_tweets = new tp_widget_recent_tweets();
		...
		$instance = array(
			'title'=> ale_get_option('footertwittertitle'),
			'consumerkey'=> ale_get_option('consumerkey'),
			'consumersecret'=> ale_get_option('consumersecret'),
			'accesstoken'=> ale_get_option('accesstoken'),
			'accesstokensecret'=> ale_get_option('accesstokensecret'),
			'cachetime'=> ale_get_option('cachetime'),
			'username'=> ale_get_option('tweetlogin'),
			'tweetstoshow'=> ale_get_option('tweetstoshow'),
			'excludereplies'=>'true',
		);
	} else { echo 'Install the "Recent Tweets Widget", please.';}
	if(get_option('tp_twitter_plugin_tweets')){
		...
		foreach($tp_twitter_plugin_tweets as $tweet){
			if(!empty($tweet['text'])){
				...
				<div class="twit">
					<p>
						<?php echo tp_convert_links($tweet['text']); ?>
					</p>
				<div class="name">
					<?php _e('@','aletheme'); ?><?php echo ale_get_option('tweetlogin'); ?>
					<span><?php echo tp_relative_time($tweet['created_at']); ?></span>
				</div>
				</div>
				<?php
...
function relproducts(){
	global $woocommerce_loop, $product,$posts_per_page,$orderby,$columns;
	...
	if ( $products->have_posts() ) : ?>
		...
		<h2><?php _e( 'Related Products', 'aletheme' ); ?></h2>
		<div class="related products  grid-shop">
...
function wc_product_sold_count() {
	...
	echo '<p>' . sprintf( __( '%s', 'aletheme' ), $units_sold ) . '</p>';
}
...
//================================================================
// PayPal Payments List - Register Sub Menu ======================
//================================================================
if( !function_exists( 'ale_register_causes_payments_page' ) ){
...
add_action('admin_menu', 'ale_register_causes_payments_page');

//================================================================
// PayPal Payments List - Display Payments =======================
//================================================================
if( !function_exists( 'ale_display_causes_payments' ) ){
	function ale_display_causes_payments(){
		?>
		<table id="payments-table" cellpadding="10px">
			...
			$paid_causes_query = new WP_Query($paid_causes_args);

			if( $paid_causes_query->have_posts() ){
				$paid_causes_query = $paid_causes_query->found_posts;
				while( $paid_causes_query->have_posts() ){
